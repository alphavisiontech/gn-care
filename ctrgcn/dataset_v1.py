...
class COCOSkeletonDataset(Dataset):
    ...
    def __init__(self, data_dir, sequence_length=30, num_classes=2, 
        ...
        if class_mapping is None:
            if num_classes == 2:
                ...
                # For 8-class training - discover classes from data
                #TODO: Implement multi-class mapping
                self.class_mapping = {}  
        else:
    ...
    def _prepare_samples(self):
                ...
                for seq_id, seq_annotations in sequences.items():
                    ...
                    for annotation in seq_annotations:
                        ...
                        if self.num_classes == 2:
                            ...
                            # Debug print
                            # print(f"Annotation category: {category_name}, classified as {action_label}")
                        else:
                            # For 8-class training, implementation can be added here 
                            #TODO: Implement multi-class label extraction
                            continue 
        ...
        if self.num_classes == 2:
            # Count samples per class for binary classification
            class_counts = {0: 0, 1: 0}
            for _, label in samples:
                class_counts[label] += 1
            print(f"Class distribution - Standing: {class_counts[0]}, Falling: {class_counts[1]}")
        else:
            # Count samples for multi-class
            class_counts = {}
            for _, label in samples:
