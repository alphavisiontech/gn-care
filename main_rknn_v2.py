    ...
    def _fall_detection_worker(self):
        ...
        while not self.stop_event.is_set():
            try:
                ...
                in_bed_status_list = [self.get_in_bed_status([p_bbox], bed_boxes) for p_bbox in person_bboxes]
                ...
                for det_idx, (ratio, score) in enumerate(zip(ratio_list, person_scores)):
                    ...
                    if ratio > self.ratio_threshold and score >= self.args.human_det_threshold and not in_bed_status:
                        if ratio == 5:
                            reason = "forward"
                        elif ratio == 6:
                            reason = "backward"
                    ...
                    if in_bed_status and score >= self.args.human_det_threshold:
                        ...
                        fall_result = { 
                            'predicted_class': "No Fall",
                            'confidence': float(max(person_scores)) if person_scores else 0.0,
                            'is_fall': False,
    ...
    try:
        while True:
            ...
            if result is not None:
                ...
                # this_frame_msg = {"timestamp": frame_timestamp, "device_id": device_id, "camera_id": camera_id,
                #           "bed_list": bed_list.tolist(), "track_id": msg_frame_track_id, "bbox": msg_frame_bbox,
                #           "skeleton": msg_frame_skeleton, "out": msg_frame_outs, "fall_filter": msg_frame_fall_filter,
                #           "wave_filter": msg_frame_wave_filter, "is_bed": msg_frame_is_bed,
                #           "Behaviour": msg_frame_behaviour, "skeleton_to_plot": skeleton_to_plot}

                # timestamp: start_time
                # device_id: device_id
                # camera_id: camera_id
                # bed_list: bed_bboxes
                # track_id: [fall_result['fall_det_index']]
                # bbox: person_bboxes[fall_result['fall_det_index']]
                # skeleton: keypoints_list[fall_result['fall_det_index']]
                # out: [] -> probability scores for each class
                # fall_filter: [] -> if the predicted_class is 'Fall', then it's true, else false 
                # wave_filter: [false]
                # is_bed: [false]
                # Behaviour: ['normal']
                # skeleton_to_plot: keypoints_list[fall_result['fall_det_index']]
...
if __name__ == "__main__":
    ...
    topic = "action_stream" # DON'T CHANGE      
    ...
    try:
        ...
        if args.input_type == 'video':
            ...
            window_name = 'PP-YOLOE + RTMPose + Fall Detection - Video (Threaded)'
            ...
            window_name = 'PP-YOLOE + RTMPose + Fall Detection - Webcam (Threaded)'
            cv2.namedWindow(window_name, cv2.WINDOW_NORMAL)
            cap, current_video_dev, current_usb_path = open_cam()
