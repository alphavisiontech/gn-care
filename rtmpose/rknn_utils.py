...
def preprocess_rtmpose(
    ...
    if bboxes is None or len(bboxes) == 0:
        ...
        bboxes = [np.array([0, 0, img_shape[1], img_shape[0]])] # h, w
...
def inference_multiple(rknn: RKNN, imgs: List[np.ndarray]) -> List[List[np.ndarray]]:
    """Inference RTMPose model for multiple people from a single image.

    Args:
        rknn: RKNN object for inference.
...
def load_rknn_model(model_path: str) -> RKNN:
    ...
    Args:
        model_path (str): RKNN model file path.

    Returns:
        rknn (RKNN): RKNN model instance.
    """
    rknn = RKNN()
